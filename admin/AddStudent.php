...
if (isset($_POST['new']) && $_POST['new'] == 1) {
    ...
    $email = $_POST['email'];
    $gender = $_POST['gender'];
    $school_id = $_POST['school_id']; // Get the selected school ID
    $profile_desc = "";
    $profile_image = "";
    $password = stripslashes($_POST['password']);
    $password = md5(mysqli_real_escape_string($con, $password));
    $role = "student";
    $ins_query = "INSERT INTO student (`full_name`,`username`,`email`,`gender`,`school_id`,`profile_desc`,`profile_image`,`password`,`role`) VALUES
    ('$full_name','$username','$email','$gender','$school_id','$profile_desc','$profile_image','$password','$role')";

    // Execute the query
    $result = mysqli_query($con, $ins_query);

    if ($result) {
        $status = "New Student Inserted Successfully. </br></br><a href='ViewStudentList.php'>View Student List</a>";
    } else {
